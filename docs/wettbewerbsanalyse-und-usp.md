...
2. **„TypeScript kann es"** heißt: *neu machbar, und inzwischen gemessen — für die Konsumenten-Skala*. Ein 35-Punkte-Sweep (`pnpm bench:scale`, 2026-07-21) hat das frühere „im Maßstab unbewiesen" durch Zahlen ersetzt: Die interaktive Latenz hält überall (warmer Hover-Median 0,04–0,11 ms bei 250 Dateien, bei 10.000-Glieder-Ketten und bei Rang 896 gleichermaßen); die Kosten der Skala landen auf dem Kaltstart (1,5 ms bei 250 Dateien, aber 10 s bei einer einzelnen 10.000-Glieder-Kette). Instantiations wachsen linear mit der Dateizahl (ca. 3.800 je Datei bei verschiedenen Shapes; wiederholte Shapes werden projektweit dedupliziert und plateauen) und linear mit der Kettenlänge (265 je Glied), aber überproportional mit dem Rang — bis der Checker bei Rang 1024 gültigen Code mit „Type instantiation is excessively deep" ablehnt (reale Arrays haben einstelligen Rang; die Wand sagt mehr über die Maschine als über den Anwendungsfall, ist aber eine Wand). **Nicht** abgedeckt ist die API-Flächen-Skala — ob die Maschinerie auch bei einer NumPy-großen Op-Menge trägt, ist eine eigene, weiterhin ungemessene Frage (§1). Details: [scale-probe-ergebnisse.md](scale-probe-ergebnisse.md).
