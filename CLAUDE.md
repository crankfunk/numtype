...
- Tuple-length arithmetic is fine for *ranks* (small ints), never for large *dimension values*. Since Kern 05, arithmetic over large literal dims IS available via digit-string types (`spike/src/slice-literal.ts`: subtraction + comparison, O(digit-count) recursion) — extendable (products for reshape/flatten would be O(digits²), see FOLLOWUPS.md), but never fall back to tuple-length-per-value. Comparison alone unlocks bounds checks incl. NEGATIVE literals (Spike 03) — check whether a use case needs only a verdict before building arithmetic.
