 ...
 * `test:core`/`test:resident`/`test:resident:gc` scripts each run
 * `node --test <explicit file list>` rather than a glob, so a new test
 * file dropped into `spike/tests-runtime/` that nobody adds to one of
 ...
 *       of `test:core` / `test:resident` (so every file runs somewhere,
 *       exactly once at that granularity — `test:resident:gc` is a
 *       deliberate re-run variant, see (c));
 *   (b) every file referenced by `test:core`, `test:resident`, or
 *       `test:resident:gc` actually exists on disk (catches stale/renamed
 *       entries left behind);
...
test("every spike/tests-runtime/*.test.ts file is listed in exactly one of test:core / test:resident", async () => {
  ...
  for (const file of onDisk) {
    const inCore = coreBasenames.has(file);
    const inResident = residentBasenames.has(file);
    if (!inCore && !inResident) {
      unregistered.push(file);
    } else if (inCore && inResident) {
      inBoth.push(file);
    }
  }
  ...
  assert.strictEqual(
    unregistered.length,
    0,
    `these test files exist in spike/tests-runtime/ but are not listed in test:core or ` +
      `test:resident (they silently never run): ${unregistered.join(", ")}`,
  );
  assert.strictEqual(
    inBoth.length,
    0,
    `these test files are listed in BOTH test:core and test:resident (should be exactly one): ${inBoth.join(", ")}`,
  );
});

test("every file referenced by test:core / test:resident / test:resident:gc exists on disk", async () => {
  const allReferenced = [
