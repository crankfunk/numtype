...
- [ ] Resident-API-Grenze: `fromArray`-Overload für `Float64Array` — `Array.from(TypedArray)` ist ~100× teurer als ein typed-array `.set()`; dadurch verliert die residente Kette 9–24 % gegen v1, obwohl die Ops selbst schneller sind (Kern-02-Befund). Design-Entscheidung: exakte API-Spiegelung vs. Boundary-Performance
